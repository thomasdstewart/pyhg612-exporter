...
class HG612Metrics:
    ...
    def fetch(self):
        ...
        self.snr_up.set(self.snr[0])
        self.snr_down.set(self.snr[1])
        self.attn_up.set(self.attn[0])
        self.attn_down.set(self.attn[1])
        self.pwr_up.set(self.pwr[0])
        self.pwr_down.set(self.pwr[1])
        self.fec_up.set(self.errors[0])
        self.fec_down.set(self.errors[1])
        self.crc_up.set(self.errors[0])
        self.crc_down.set(self.errors[1])
    ...
    def get(self):
        ...
        host = int(os.getenv('HOST', 'modem'))
        user = int(os.getenv('USER', 'admin'))
        password = int(os.getenv('PASSWORD', 'admin'))
    ...
    def parse(self):
        ...
        CRC:		492		1238
        '''
        self.maxrate = re.findall('Max:\s+Upstream rate = (\d+) Kbps, Downstream rate = (\d+) Kbps', self.raw_data)[0]
        self.bearerrate = re.findall('Bearer:\s+0, Upstream rate = (\d+) Kbps, Downstream rate = (\d+) Kbps', self.raw_data)[0]
        self.snr = re.findall('SNR \(dB\):\s+([.\d]+)\s+([.\d]+)+', self.raw_data)[0]
        self.attn = re.findall('Attn\(dB\):\s+([.\d]+)\s+([.\d]+)+', self.raw_data)[0]
        self.pwr = re.findall('Pwr\(dBm\):\s+([.\d]+)\s+([.\d]+)+', self.raw_data)[0]
        self.errors = re.findall('Total time = .*\r\nFEC:\s+(\d+)\s+(\d+)\r\nCRC:\s+(\d+)\s+(\d+)', self.raw_data)[0]
